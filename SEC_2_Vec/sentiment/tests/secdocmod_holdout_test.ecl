...
get_info(INTEGER n) := FUNCTION
    sm_pl := secdocmod_n(n,'plain');
    sm_sp := secdocmod_n(n,'s&p');
    sec := sm_pl.s;
    ...
    dppl := sm_pl.dp[1].pod;
    dpsp := sm_sp.dp[1].pod;
    hppl := sm_pl.dph[1].pod;
    hpsp := sm_sp.dph[1].pod;
    dpel := sm_pl.dp[1].pode;
    desp := sm_sp.dp[1].pode;
    dhpl := sm_pl.dph[1].pode;
    dhsp := sm_sp.dph[1].pode;

    RETURN DATASET([{sec,apl,asp,
                    ppl,psp,hpl,hsp,
                    pepl,pesp,hepl,hesp,
                    dppl,dpsp,hppl,hpsp,
                    dpel,desp,dhpl,dhsp}],outrec);
END;
