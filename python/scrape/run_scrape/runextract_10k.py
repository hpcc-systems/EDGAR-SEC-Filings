...
import pandas as pd
from useapi import *
from label_filename_10k import *
...
for i in range(len(ticklist)):
  ...
  scrape_path = '/Users/Matthias/Documents/LexisNexis/SEC_10qs/scraped_10ks/'

  for x in xmldata:
    fname = x[2].lower()+'_'+fixdate(x[-1])+'_10k.xml'
    ...
    try:
      testdir = os.listdir(scrape_path+foldpath(sectlist[i]))
    except:
      os.mkdir(scrape_path+foldpath(sectlist[i]))
